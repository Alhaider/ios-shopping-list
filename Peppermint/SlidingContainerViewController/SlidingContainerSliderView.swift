...
class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
    ...
    init (width: CGFloat, titles: [String]) {
        ...
        appearance = SlidingContainerSliderViewAppearance (
            backgroundColor: UIColor(red: CGFloat(216.0/255.0), green: CGFloat(216.0/255.0), blue: CGFloat(216.0/255.0), alpha: CGFloat(1.0)),
            
            font: UIFont (name: "HelveticaNeue-Light", size: 20)!,
            selectedFont: UIFont.systemFont(ofSize: 20),
            ...
            selectorColor: UIColor(red: CGFloat(28.0/255.0), green: CGFloat(202.0/255.0), blue: CGFloat(135.0/255.0), alpha: CGFloat(1.0)),
            selectorHeight: 5,
            fixedWidth: false)
    ...
    func draw () {
        ...
        if appearance.fixedWidth {
            ...
            for title in titles {
                ...
                label.center.y = frame.size.height/2 + 10.0
                labelTag += 1
                label.tag = labelTag
                
                let touchView = UIView (frame: CGRect (origin: CGPoint(x: currentX, y: 0.0), size: CGSize(width: label.frame.size.width, height: 65.0)))
                touchView.isUserInteractionEnabled = true
                touchView.tag = labelTag
                touchView.addGestureRecognizer(UITapGestureRecognizer (target: self, action: #selector(SlidingContainerSliderView.didTap(_:))))
                addSubview(label)
